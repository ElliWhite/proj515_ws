...
int main(int argc, char** argv){
  ...
  ros::init(argc, argv, "path_plotter");    // Initialise ROS Node
  ...
  while (nh.ok()){
    ...
    try{
      ...
      listener.lookupTransform("map", "base_link", ros::Time(0), transform_msg);        // Save the transform into transform_msg
    }
